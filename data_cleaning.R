...
### Carga datos generales tratamiento y une bases
...
data_clean_nd <- data_clean_nd %>% 
  ...
  mutate(edad = 2020 - year(fecha_nacimiento),
         ...
         edad_dx = interval(fecha_nacimiento, fecha_dx_CA1) %>% 
           as.duration() %/% dyears()) %>% 
  ## Gravedad del cáncer
  mutate(gravedad_CA = if_else(grepl("Dist|Reg", evol_CA1)|grepl("Dist|Reg", evol_CA2),
                          "Grave", "Leve"),
         ...
         metastasis = case_when(
           ...
           TRUE ~ "No")) %>% 
  ## Tiempo entre diagnóstico de cáncer y COVID
  ...
  ## Ordena variables
  select(idpte, fecha_nacimiento, edad, edad_cat, sexo, starts_with("comorb"),
         ...
         fecha_dx_covid, dx_covid, dx_covid_estudio, dias_CA_covid, dias_ini_covid)
